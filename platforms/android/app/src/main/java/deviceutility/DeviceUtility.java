...
public class DeviceUtility extends CordovaPlugin {
    private String TAG = DeviceUtility.class.getSimpleName();
    private CordovaInterface cordova;
    private CallbackContext callbackContext;
    private String action;
    private JSONArray data;
    private int requestedPermissionCount = 0;

    private static final String FIND_SSID = "findSSID";
	...
	if(action.equals("grantPermission")) {
            return this.grantPermission(callbackContext);
        } else if(action.equals("getLocationPermissionStatus")) {
            return this.getLocationPermissionStatus(callbackContext);
        ...
        } else if(action.equals(FIND_SSID)) {
            return this.findSSID(callbackContext, data);
    ...
    private boolean getLocationPermissionStatus(CallbackContext callbackContext) {
	int hasPermission = 1;
	if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.M){
	    hasPermission = cordova.hasPermission
		(Manifest.permission.ACCESS_COARSE_LOCATION) ? 1:0;
	}
	...
	if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.M){
		...
		if (!cordova.hasPermission(permission)) {
		    permissionsList = addStringArray(permissionsList, permission);
		}
		...
		if (permissionsList.length != 0) {
		    requestedPermissionCount = permissionsList.length;
		    cordova.requestPermissions(this, 0, permissionsList);
		}
		Log.d(TAG, "request " + requestedPermissionCount + " permission");
		callbackContext.success("request " + requestedPermissionCount + " permission");
	} else{
	    // do something for phones running an SDK before lollipop
		Log.d(TAG, "OS below M: " + android.os.Build.VERSION.SDK_INT);
		callbackContext.success("OS below M: " + android.os.Build.VERSION.SDK_INT);
	}
        return true;
    }

    private String[] addStringArray(String[] permissionsList, String permission) {
        int len = permissionsList.length;
        String[] newArray = new String[len+1];
        for (int i=0; i<len; i++) {
            newArray[i] = permissionsList[i];
        }
        newArray[len] = permission;
        return newArray;
    }
    ...
    private boolean grantPermission(CallbackContext callbackContext) {
...
/// WiFI
     ...
     *
     *    @param    callbackContext        A Cordova callback context
     *    @param    data                JSON Array, with [0] being SSID to Find
     *    @return    true if SSID is found, false if not found
     */
    private boolean findSSID(CallbackContext callbackContext, JSONArray data) {
        Log.d(TAG, "WifiWizard: findSSID entered.");

        if(!validateData(data)) {
            Log.d(TAG, "WifiWizard: findSSID invalid data.");
            callbackContext.error("WifiWizard: findSSID invalid data");
            return false;
        }
        //Log.d(TAG, "WifiWizard: After validateData.");
        String ssidToFind = "";
        try {
            ssidToFind = data.getString(0).replaceAll("\"", "");
            //Log.d(TAG, "WifiWizard: ssidToFind: " + ssidToFind);
        }
        catch (Exception e) {
            Log.d(TAG, e.getMessage());
            callbackContext.error(e.getMessage());
        ...
        //Log.d(TAG, "WifiWizard: After validateData.");
        if (ssidToFind.length() > 0) {
            if (wifiManager.startScan()) {
                List<ScanResult> scanResults = wifiManager.getScanResults();
                //Log.d(TAG, "WifiWizard:" + ssidToFind + ":After getScanResults: " + scanResults.size());
                for (ScanResult scan : scanResults) {
                    //Log.d(TAG, "WifiWizard: scan.SSID: " + scan.SSID);
                    if (scan.SSID.contains(ssidToFind)) {
                        Log.d(TAG, "WifiWizard: FOUND " + ssidToFind + " in scan.SSID: " + scan.SSID);
                        String new_string = scan.SSID.replace(ssidToFind, "");
                        callbackContext.success(new_string);
                        return true;
                    }
                }
            }
        ...
        callbackContext.error("WifiWizard: cannot find SSID: " + ssidToFind);
        return true;
    }
